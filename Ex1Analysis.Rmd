...
=============================================
```{r}
#data<-subset(data,cond%in%c("a","c","d","f"))
#unique(data$expt)
```
...
```{r}
# Raw rt version
rt_data_summary <- ddply(rtg, .(cond,region), summarise, N=length(rt), mean=mean(rt), sd=sd(rt), se=sd/sqrt(N))
```
...
g <- g + scale_shape_manual(values=c(21,22,24,25))
...
g <- g + scale_shape_manual(values=c(22,21))
...
#g <- g + theme_classic(base_size=20, base_family="HiraKakuProN-W3")
g <- g + theme_classic(base_size=20, base_family="Times")
g <- g + labs(x="Region", y="Transformed RT")
print(g)
```
...
Add error bars
```{r}
g <- g + geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.1)
```
...
#g <- g + theme_classic(base_size=20, base_family="HiraKakuProN-W3")
g <- g + theme_classic(base_size=20, base_family="Times")
g <- g + labs(x="Temporal Adv", y="RT (ms)")
print(g)
```
...
rt_data$ab<-ifelse(rt_data$cond%in%c("a","b"),0.5,-0.5)
rt_data$ac<-ifelse(rt_data$cond%in%c("a","c"),0.5,-0.5)
rt_data$int<-rt_data$ab*rt_data$ac

# For pairwise comparisons
rt_data$simpleVSredup_dur <-ifelse(rt_data$ab==0.5,
                          ifelse(rt_data$ac==0.5,0.5,-0.5),
                          0)
rt_data$simpleVSredup_nondur <-ifelse(rt_data$ab==-0.5,
                          ifelse(rt_data$ac==0.5,0.5,-0.5),
                          0)
rt_data$durVSnondur_simple <-ifelse(rt_data$ac==0.5,
                          ifelse(rt_data$ab==0.5,0.5,-0.5),
                          0)
rt_data$durVSnondur_redup <-ifelse(rt_data$ac==-0.5,
                          ifelse(rt_data$ab==0.5,0.5,-0.5),
                          0)
...
rt_data check
```{r}
dc <- subset(rt_data,cond=="a",c(cond,word,ab,ac,int))
head(dc, n=7)
dc <- subset(rt_data,cond=="b",c(cond,word,ab,ac,int))
head(dc, n=7)
dc <- subset(rt_data,cond=="c",c(cond,word,ab,ac,int))
head(dc, n=7)
dc <- subset(rt_data,cond=="d",c(cond,word,ab,ac,int))
head(dc, n=7)
...
##### Max model
```{r}
#mm<-lmer(rt~ab*ac+spillover+
#           (ab*ac+spillover+1|subj)+(ab*ac+spillover+1|item),
#           control=lmerControl(optimizer="bobyqa"),
#           cregion)

#mm<-lmer(rt~ab*ac+
#           (ab*ac+1|subj)+(ab*ac+1|item),
#           control=lmerControl(optimizer="bobyqa"),
#           cregion)
...
```{r}
mmm<-lmer(rt~simpleVSredup_dur+simpleVSredup_nondur+ab+
           (ab*ac+1|subj)+(ab*ac+1|item),
           control=lmerControl(optimizer="bobyqa"),
           cregion)
summary(mmm)

mmm<-lmer(rt~durVSnondur_simple+durVSnondur_redup+ac+
           (ab*ac+1|subj)+(ab*ac+1|item),
           control=lmerControl(optimizer="bobyqa"),
           cregion)
summary(mmm)
...
##### Post hoc multiple pairwise comparisons to test the interaction
```{r}
mmm<-lmer(rt~simpleVSredup_dur+simpleVSredup_nondur+ab+spillover+
           (1|subj)+(1|item),
           control=lmerControl(optimizer="bobyqa"),
           cregion)
summary(mmm)

mmm<-lmer(rt~durVSnondur_simple+durVSnondur_redup+ac+spillover+
           (1|subj)+(1|item),
           control=lmerControl(optimizer="bobyqa"),
           cregion)
summary(mmm)
