...
##### Load data (including user information):

```{r}
#alldata<-read.csv("Ex2alldata.csv",header=T)
```

#### Drop columns: age, language, lancers username (for paper)

```{r}
#alldata <- alldata[, -which (colnames(alldata) %in% c("age", "language", "lancers"))]
#write.csv(alldata,file="Ex2alldata_anon.csv",quote=F)
alldata <- read.csv("Ex2alldata_anon.csv", header = T)
```
...
print(g)
ggsave(file = "Fig6.png", plot = g, dpi = 300)
```
...
print(g)
ggsave(file = "Fig7.png", plot = g, dpi = 300)
```
...
print(g)
ggsave(file = "Fig8.png", plot = g, dpi = 300)
```
...
g <- g + labs(x="Temporal adverb", y=RTlabel)
print(g)
ggsave(file = "Fig9.png", plot = g, dpi = 300)
```
